...
export class ShoppingBagListViewModel {
    ...
    constructor(context: Context) {
        ShoppingBagConstants.PRODUCT_NAME_LIST(context).forEach((item: string, index: number) => {
            this.ShoppingBagList.push(new ShoppingBagProduct(item, ShoppingBagConstants.PRODUCT_IMAGE_LIST[index], ShoppingBagConstants.PRODUCT_TYPE_LIST(context)[index], ShoppingBagConstants.PRODUCT_PREFERENTIALList_LIST(context)[index], ShoppingBagConstants.PRODUCT_PRICE_LIST[index], ShoppingBagConstants.PRODUCT_ORIGINAL_PRICE_LIST[index], ShoppingBagConstants.PRODUCT_PRESENT_LIST(context)[index], ShoppingBagConstants.PRODUCT_SERVICE_LIST(context)[index]));
        });
    }
...
export class ShoppingBagProduct {
    ...
    constructor(name: string, image: Resource, type: string, preferentialList: Array<string>, price: number, originalPrice: number, present: string, service: string) {
        this.name = name;
        this.image = image;
        this.type = type;
